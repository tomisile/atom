...
PYTHON_SCRIPT="today.py"
PYTHON_ENV="tfenv"  # Your conda environment name
LOG_FILE="$HOME/Projects/pred-bot/today_local.log"
...
validate_slack_config() {
    ...
    if [[ -z "$SLACK_GENERAL_CHANNEL_ID" ]]; then
        log_message "ERROR: SLACK_GENERAL_CHANNEL_ID not found in environment"
        exit 1
    fi
...
EOF
    ...
    log_message "Sending message to Slack channel: $SLACK_GENERAL_CHANNEL_ID"
